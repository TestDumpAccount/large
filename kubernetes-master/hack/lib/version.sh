...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='d89d5ab2680bc74fe4487ad71e514f4e0812d9ce'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'HEAD -> master, refs/__gh__/pull/92316/rebase, refs/__gh__/pull/114712/rebase' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'HEAD -> master, refs/__gh__/pull/92316/rebase, refs/__gh__/pull/114712/rebase' =~ tag:\ (v[^ ,]+) ]]; then
